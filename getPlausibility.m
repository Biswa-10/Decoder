function plausibility = getPlausibility(encodedMsg,map,symbols,numberOfSymbolsOriginal,transMatOrder1,transMatOrder2)
...
%Given a mapping function, the getPlausibility function evaluates how closely it resembles to our original transition matrix. It returns a numerical value that it associates with the plausibility.
    ...
    for i = 1:(strlength(encodedMsg)-2)
        symLoc1 = symbolLocation(encodedMsg(i),symbols,numberOfSymbolsOriginal);
        symLoc2 = symbolLocation(encodedMsg(i+1),symbols,numberOfSymbolsOriginal);
        symLoc3 = symbolLocation(encodedMsg(i+2),symbols,numberOfSymbolsOriginal);
        x1=map(1,symLoc1);
        x2=map(1,symLoc2);
        x3=map(1,symLoc3);
